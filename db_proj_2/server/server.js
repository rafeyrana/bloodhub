...
app.post("/api/signup", (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashed_password) => {
        if (err) throw err;
        connection.query(`INSERT INTO users (CNIC,Email ,Password ,Phone_Number ,First_Name ,Middle_Name ,Last_Name ,Approved ,Receiver_Request_Counter ,Admin,City)
        values("${cnic}","${email}","${hashed_password}","${phone_num}","${first_name}","${middle_name}","${last_name}",0,0,0,"${city}");`, (err, result) => {
            ...
            if (err){
                ...
                connection.query(`INSERT INTO medical_records (Age, Weight, Height, CNIC, Blood_Group) values(${age},${weight},${height},"${cnic}","${blood_type}");`, (err, result) => {
                    if (err) console.log(err);
                    else {
                        if (medical_condition1 != "") {
                            connection.query(`INSERT INTO medical_conditions (user_condition, CNIC) values("${medical_condition1}","${cnic}")`, (err, mc_result) => {
                        ...
                        if (medical_condition2 != "") {
                            connection.query(`INSERT INTO medical_conditions (user_condition, CNIC) values("${medical_condition2}","${cnic}")`, (err, mc_result) => {
                        ...
                        if (medical_condition3 != "") {
                            connection.query(`INSERT INTO medical_conditions (user_condition, CNIC) values("${medical_condition3}","${cnic}")`, (err, mc_result) => {
...
app.post("/api/login",(req, res) =>{
    ...
    connection.query(`select count(*) as count,approved,password  from users where email = "${email}"`, (err,output)=>{
        if (err) console.log(err);
        else {
            ...
            if(output[0].count !=0){
                if(output[0].approved == 1){
                    ...
                    bcrypt.compare(password, correct_password_hash, (err, comparison_result)=>{
                        if (err) throw err
